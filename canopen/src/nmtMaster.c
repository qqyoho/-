...
extern e_nodeState NMTable[NMT_MAX_NODE_ID]; 

/******************************************************************************/
UNS8 masterSendNMTstateChange(UNS8 Node_ID, UNS8 cs)
{
	Message m;
...
/****************************************************************************/
UNS8 masterSendNMTnodeguard(UNS8 nodeId)
{
	Message m;
...
/******************************************************************************/
void masterRequestNodeState(UNS8 nodeId)
{
	...
	NMTable[nodeId] = Unknown_state; /* A state that does not exist */

	if (nodeId == 0) { /* NMT broadcast */
		UNS8 i = 0;
		for (i = 0 ; i < NMT_MAX_NODE_ID ; i++) {
			NMTable[i] = Unknown_state;
		}
	}
	masterSendNMTnodeguard(nodeId);
}
